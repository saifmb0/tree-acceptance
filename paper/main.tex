...
\author{
  ...
  \IEEEauthorblockA{Al Ain University \\
...
across cognitively distinct task domains. Domain-adaptive speculative decoding
has been studied in the context of technical specialization — e.g., function
calling, biology, and domain-specific language models — where the draft model
is fine-tuned to match a specialized target \cite{hu2025training}. These works focus on closing
the distribution gap through training rather than characterizing how cognitive
task type affects acceptance under a fixed, general-purpose draft model. Our
work is orthogonal: we hold the draft-target pair constant and vary the task
domain, isolating task type as an independent variable in acceptance dynamics.
...
\textbf{Target model:}
\texttt{TheBloke/Llama-2-7B-Chat-GPTQ}~\cite{touvron2023llama},
...
\textbf{Draft model:}
\texttt{TinyLlama/TinyLlama-1.1B-Chat-v1.0}~\cite{zhang2024tinyllama},
...
generation step within the output sequence affects acceptance. We partition
token positions into two equal bins: early positions (bin~0, tokens 1--32)
and late positions (bin~1, tokens 33--64), and report $\bar\alpha$ per
(depth, bin) cell in Table~\ref{tab:pos}.
...
later in the sequence. This effect is most pronounced at depth~1
($+0.028$), where the output has already established a topic, style, and
syntactic register. Once the model has committed to a generation direction
at earlier positions, the marginal next-token prediction task at depth~1
becomes easier for both models, increasing the probability that the
draft's choice coincides with the target's preference.

The depth-2 row is a near-zero exception ($-0.003$), consistent with
the interpretation that depth-2 nodes are mid-chain syntactic complements
whose predictability is largely determined by their parent node rather
...
that requires no re-training: classifying incoming requests by task type
before deciding whether to activate speculative decoding at all. A
lightweight prompt classifier (even a rule-based heuristic) can segregate
chat and conversational requests---for which $E[L] > 1$ is confirmed---from
mathematical or code requests, where the current draft provides no net
speedup. Disabling speculation for the low-$E[L]$ tail eliminates the
cost of draft tree construction on requests where it actively hurts latency,
without modifying the models themselves. This approach is trivially
composable with existing inference-serving infrastructure and represents
an immediately deployable recommendation from our findings.
